    ...
    <div className="space-y-8" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <Card className="p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          ...
          >
            {showAdvancedOptions ? (language === 'ar' ? 'إخفاء الخيارات المتقدمة' : 'Hide Advanced Options') : (language === 'ar' ? 'خيارات متقدمة' : 'Advanced Options')}
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {contentOptions.map(option => (
              <button
                ...
                className={`p-4 rounded-lg border-2 transition-all duration-200 text-left ${exportOptions?.content === option.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'}`}
              >
            ...
            {exportFormats.map(format => {
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all duration-200 text-left ${exportOptions?.format === format.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'}`}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {timeRangeOptions.map(option => (
                  <button
                    ...
                    className={`p-3 rounded-lg border-2 transition-all duration-200 text-left ${exportOptions?.timeRange === option.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'}`}
                  >
                    <div className="font-medium text-gray-900 dark:text-white">{option.name}</div>
                    <div className="text-sm text-gray-500 dark:text-gray-400">{option.description}</div>
                  </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {languageOptions.map(option => (
                  <button
                    ...
                    className={`p-3 rounded-lg border-2 transition-all duration-200 text-left ${exportOptions?.exportLanguage === option.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'}`}
                  >
